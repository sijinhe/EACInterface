...
function constructor() {
    ...
    if (Websock_native) {
        ...
        updateState('loaded', 'Fancy-VNC ready: native WebSockets, ' + rmode);
    } else {
        ...
        if ((! Util.Flash) ||
            ...
            updateState('loaded', 'Fancy-VNC ready: WebSockets emulation, ' + rmode);
        }
    }
    ...
    if (state in {'disconnected':1, 'loaded':1, 'connect':1,
        ...
        if (display && display.get_context()) {
            ...
            if ((Util.get_logging() !== 'debug') ||
                (state === 'loaded')) {
                // Show Fancy-VNC logo on load and when disconnected if
                // debug is off
                display.clear();
            }
        }
